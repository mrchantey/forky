...
impl AssetIo for WebAssetIo {
		...
		if is_http(path) {
			...
			let fut = Box::pin(async move {
				...
				if let Err(err) = &response {
					log!("Failed to fetch asset {uri}: {err:?}");
				}
		...
		if is_http(path) {
			...
			if let Ok(mut filesystem_watcher) = self.filesystem_watcher.write()
			{
				if let Some(ref mut watcher) = *filesystem_watcher {
					watcher.watch(&absolute_path).map_err(|_error| {
						AssetIoError::PathWatchError(absolute_path)
					})?;
				}
			}
	...
	fn get_metadata(
		&self,
		path: &Path,
	) -> Result<bevy::asset::Metadata, AssetIoError> {
