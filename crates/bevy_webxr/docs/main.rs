...
use kiss_engine_wgpu::{
	BindGroupLayoutSettings, BindingResource, Device, RenderPipelineDesc,
	Texture, VertexBufferLayout,
};
use rand::Rng;
...
async fn run() {
	#[cfg(feature = "wasm")]
	{
		...
		let level: log::Level =
			wasm_web_helpers::parse_url_query_string_from_window("RUST_LOG")
	...
	*/
	...
	let webgl2_context = {
		...
		let js_gl_attribs =
			wasm_bindgen::JsValue::from_serde(&gl_attribs).unwrap();
	...
	let (plane, _) =
		Model::new(include_bytes!("../plane.glb"), &device, "plane");
	...
	let (water, _) =
		Model::new(include_bytes!("../water.glb"), &device, "water");
	let (tree, _) = Model::new(include_bytes!("../tree.glb"), &device, "tree");
	let (house, _) =
		Model::new(include_bytes!("../house.glb"), &device, "house");

	let (meteor, _) =
		Model::new(include_bytes!("../meteor.glb"), &device, "meteor");
	...
	let uniform_buffer =
		device.create_resource(device.inner.create_buffer_init(
	...
	*/
	...
	let sampler = device.create_resource(device.inner.create_sampler(
	...
	let linear_sampler = device.create_resource(device.inner.create_sampler(
		&wgpu::SamplerDescriptor {
			address_mode_u: wgpu::AddressMode::Repeat,
			address_mode_v: wgpu::AddressMode::Repeat,
	...
	let joint_transforms_buffer = device.create_resource(
	...
	run_rendering_loop(&xr_session, move |time, frame| {
		...
		*/
		...
		// update
		{
			...
			let parse_matrix = |vec| {
				Mat4::from_cols_array(&<[f32; 16]>::try_from(vec).unwrap())
			};

			let left_proj = parse_matrix(views[0].projection_matrix());

			let left_inv =
				parse_matrix(views[0].transform().inverse().matrix());
			...
			if let Some(right_view) = views.get(1) {
				let right_inv =
					parse_matrix(right_view.transform().inverse().matrix());
		...
		let mut encoder = device.inner.create_command_encoder(
...

		{
			...
			let device = device
				.with_formats(formats, Some(wgpu::TextureFormat::Depth32Float));
...
impl HeightMap {
	fn sample(&self, position: Vec2) -> f32 {
		let position =
			position * Vec2::new(self.width as f32, self.height as f32);
		...
		fn lerp(x: f32, y: f32, f: f32) -> f32 { x * (1.0 - f) + y * f }

		let other_x = (x + 1).min(self.width - 1);
		let other_y = (y + 1).min(self.height - 1);

		let row_1 =
			lerp(self.fetch(x, y), self.fetch(other_x, y), fractional_x);
		let row_2 = lerp(
...
impl State {
	...
	fn update(
		...
		if (forwards, right) != (0, 0) {
			...
			self.player_facing +=
				short_angle_dist(self.player_facing, new_player_facing) * 0.5;

			self.orbit.yaw +=
				short_angle_dist(self.orbit.yaw, self.player_facing) * 0.015;
		...
		self.bounce_sphere_props.scale +=
			self.bounce_sphere_props.scale * delta_time * 0.5;
		...
		if (self.meteor_props.position.x
			+ self.meteor_props.velocity.x * delta_time)
			.abs() > 40.0
		{
			self.meteor_props.velocity.x *= -1.0;
		}

		if (self.meteor_props.position.z
			+ self.meteor_props.velocity.z * delta_time)
			.abs() > 40.0
		{
		...
		{
			self.bounce_sphere_props.scale = 1.0;
			self.bounce_sphere_props.position =
				self.player_position_3d(heightmap) + Vec3::Y * 4.0;
		}

		self.animation_time += delta_time;

		while self.animation_time
			> player_model.animations[self.animation_id].total_time()
		{
			self.animation_time -=
				player_model.animations[self.animation_id].total_time();
...
async fn button_click_future(button: &web_sys::HtmlButtonElement) {
	wasm_bindgen_futures::JsFuture::from(js_sys::Promise::new(
		&mut |resolve, _reject| button.set_onclick(Some(&resolve)),
	))
	.await
	.unwrap();
}
