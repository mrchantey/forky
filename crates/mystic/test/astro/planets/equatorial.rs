...
sweet! {
	it "works" {
		...
		expect(ecliptic_positions::ecliptic_position(day,Planet::Moon).to_geo(day).to_equatorial(day,&position))
			.to_be(EquatorialCoords { right_ascention: 14.417484465170674, declination: -8.596795754328033, radius: 0.0026780889984901832 })?;
		expect(ecliptic_positions::ecliptic_position(day,Planet::Mercury).to_geo(day).to_equatorial(day,&position))
			.to_be(EquatorialCoords { right_ascention: 18.08274238082684, declination: -24.380635981533466, radius: 1.4131239881531124 })?;
		expect(ecliptic_positions::ecliptic_position(day,Planet::Venus).to_geo(day).to_equatorial(day,&position))
		...
		expect(ecliptic_positions::ecliptic_position(day,Planet::Jupiter).to_geo(day).to_equatorial(day,&position))
			.to_be(EquatorialCoords { right_ascention: 1.5913431171387373, declination: 8.59473659329149, radius: 4.6151445225085475 })?;
		expect(ecliptic_positions::ecliptic_position(day,Planet::Saturn).to_geo(day).to_equatorial(day,&position))
			.to_be(EquatorialCoords { right_ascention: 2.584700779979909, declination: 12.608724369868964, radius: 8.643001983947073 })?;
		expect(ecliptic_positions::ecliptic_position(day,Planet::Uranus).to_geo(day).to_equatorial(day,&position))
			.to_be(EquatorialCoords { right_ascention: 21.163085734920678, declination: -17.02975121085124, radius: 20.720022432032987 })?;
		expect(ecliptic_positions::ecliptic_position(day,Planet::Neptune).to_geo(day).to_equatorial(day,&position))
			.to_be(EquatorialCoords { right_ascention: 20.36083801993554, declination: -19.218838313378413, radius: 31.004809501352373 })?;
		expect(ecliptic_positions::ecliptic_position(day,Planet::Pluto).to_geo(day).to_equatorial(day,&position))
